...
wchar_t *apply_hashtag(wchar_t *str, flag_t *flag)
{
    ...
    if (!is_char_in_wstr(L"oxX", flag->flag_struct->conversion_specifier)) {
        return str;
    }
...
{
    if (is_char_in_wstr(L"csnbS%", flag->flag_struct->conversion_specifier)) {
        return str;
    }
    ...
    if (!is_char_in_wstr(L"difFeEgGaA", flag->flag_struct->conversion_specifier)) {
        return str;
    }
...
static size_t where_to_insert_padding(wchar_t *str, flag_t *flag, wchar_t c)
{
    size_t n = 0;
...
{
    bool is_zero = flag->flag_struct->has_zero;
    is_zero *= !flag->flag_struct->has_hyphen;
    is_zero *= !is_char_in_wstr(L"%csSn", flag->flag_struct->conversion_specifier);
    return is_zero ? '0' : ' ';
}
...
wchar_t *apply_padding(wchar_t *str, flag_t *flag)
{
    ...
    const size_t index = where_to_insert_padding(str, flag, padding_char);
    size_t str_length = my_wstrlen(str);
