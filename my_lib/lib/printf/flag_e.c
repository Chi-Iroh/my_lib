...
static wchar_t *append_exponent(wchar_t *str, short exponent, bool is_lower)
{
    ...
    str = wstr_append(str, is_lower ? 'e' : 'E');
...
(long double n, unsigned decimals, bool is_lower, bool has_hashtag)
{
    ...
    if (n < 0.L) {
        str = wstr_append(str, '-');
        positive_call = flag_e_impl(-n, decimals, is_lower, has_hashtag);
    ...
    str = flag_f(n, decimals, is_lower, has_hashtag);
    return append_exponent(str, exp, is_lower);
}
...
(long double n, unsigned decimals, bool is_lower, bool has_hashtag)
{
    if (my_isnan(n)) {
        ...
        return flag_e_impl(n, decimals, is_lower, has_hashtag);
    }
}
