...
{
    const bool hex_lower = flag->flag_struct->conversion_specifier == 'x';
    const unsigned int_precision = get_int_precision(flag);
...
{
    const unsigned precision = get_str_precision(flag);
